...
class GeminiLive:
    ...
    Supports dynamic voice, system instruction, and language configuration per agent.
    """
    ...
    async def start_session(
        ...
        logger.info(
            f"Connecting to Gemini Live | model={self.model} | voice={self.voice_name}"
        )
        try:
                    ...
                    try:
                        while True:
                            async for response in session.receive():
                                ...
                                if response.go_away:
                                    logger.warning(f"GoAway: {response.go_away}")
